...
ForwardCompositionalSE3::ForwardCompositionalSE3(
    ...
    Sophus::SE3d& init_model, int n_levels, int n_iter, float res_thresh,
...
void ForwardCompositionalSE3::runOptimization(Sophus::SE3d& model,
                                              int levelBegin, int levelEnd) {
  if (levelBegin < 0 || levelBegin > n_levels_ - 1) levelBegin = n_levels_ - 1;
...
double ForwardCompositionalSE3::computeResiduals(const Sophus::SE3d& model,
...
double SecondOrderMinimisationSE3::computeResiduals(const Sophus::SE3d& model,
                                                    bool linearize_system,
                                                    bool compute_weight_scale) {
  // Warp the image such that it aligns with the template image
  double chi2 = 0;
  ...
  // Compute Jacobian
  if (linearize_system) resimg_ = cv::Mat_<float>(tpl_pyr_[level_].size(), 1.0);
